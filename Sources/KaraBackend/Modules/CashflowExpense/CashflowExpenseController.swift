...
struct CashflowExpenseController: RouteCollection {
    ...
    func boot(routes: any RoutesBuilder) throws {
        ...
        cashflowExpenses.get(":shopId", ":id", use: findByIdAndShop)
    ...
    func findByIdAndShop(req: Request) async throws -> CashflowExpenseResponseDTO {
        guard let shopId = req.parameters.get("shopId", as: UUID.self) else {
            throw Abort(.badRequest, reason: "Invalid shop ID")
        }
        ...
        return try await cashflowExpenseService.findByIdAndShop(
            id,
            shopId: shopId,
            on: req.db
        )
